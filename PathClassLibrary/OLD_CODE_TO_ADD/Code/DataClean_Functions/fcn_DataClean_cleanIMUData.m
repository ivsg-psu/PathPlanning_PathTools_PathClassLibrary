function cleaned = fcn_DataClean_cleanIMUData(d)
...
% cleaned.ZGyro = fcn_DataClean_medianFilterData(cleaned.ZGyro,2*cleaned.ZGyro_Sigma);
% 
...
% cleaned.XAccel = fcn_DataClean_medianFilterData(cleaned.XAccel,2*cleaned.XAccel_Sigma);
...
% cleaned.YAccel = fcn_DataClean_medianFilterData(cleaned.YAccel,2*cleaned.YAccel_Sigma);
...
% cleaned.ZAccel = fcn_DataClean_medianFilterData(cleaned.ZAccel,2*cleaned.ZAccel_Sigma);
% cleaned.ZAccel_Sigma = std(diff(cleaned.ZAccel)); % Calculate the variance in the data

end

%% Subfunctions start here
function cleaned = fcn_DataClean_medianFilterData(data,sigmas)
...
function d = fcn_DataClean_uniformSample(d)
