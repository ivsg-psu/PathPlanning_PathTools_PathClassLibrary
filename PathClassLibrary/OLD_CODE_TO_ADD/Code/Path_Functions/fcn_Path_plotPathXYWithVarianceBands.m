function fcn_Path_plotPathXYWithVarianceBands(reference_traversal, varargin)
...
if flag_check_inputs
    ...
    try
        ...
        error('The reference_traversal input must be a structure containing the fields X, Y, and Station');
    end
    
    if (length(X_reference(:,1))~=length(Y_reference(:,1))) || ((length(X_reference(:,1))~=length(Station_reference(:,1))))
        error('X, Y, and Station fields in the reference_traversal structure must have the same length, and must be N x 1 vectors');
    end
    
    if ~issorted(Station_reference,'strictascend')
        error('The station field on the reference_traversal must be increasing and contain no duplicates');
    end
