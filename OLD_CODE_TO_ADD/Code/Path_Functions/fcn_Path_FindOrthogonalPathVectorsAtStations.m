...
X_central_at_stations = interp1(Station_central,X_central,stations,'linear');
Y_central_at_stations = interp1(Station_central,Y_central,stations,'linear');
...
Indices_Central_at_Query_Stations = interp1(Station_central, Indices_central,stations);
...
start_query_index = find(stations==Station_central(1));
if ~isempty(start_query_index)
    indices_start(start_query_index) = 1;
    indices_end(start_query_index) = 2;
end

end_query_index = find(stations==Station_central(end));
...
if flag_do_plots
    ...
    quiver(X_central_at_stations,Y_central_at_stations,normal_unit_vectors_at_stations(:,1),normal_unit_vectors_at_stations(:,2),'g','Linewidth',3);
