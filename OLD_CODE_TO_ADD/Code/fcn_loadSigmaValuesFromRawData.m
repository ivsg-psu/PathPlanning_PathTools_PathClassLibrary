function RawDataWithSigmas = fcn_loadSigmaValuesFromRawData(rawData)
% fcn_loadSigmaValuesFromRawData - ensures that key variables have standard
    ...
    for i_subField = 1:length(subfieldNames)
        ...
        if any(strcmp(subFieldName,fields_to_calculate_sigmas_for))
            ...
            if any(strcmp(subFieldNameSigma,subfieldNames))
                ...
                % consider this.
                data = d.(subFieldName);
                real_sigma = fcn_calcSigmaNoOutliers(data);
                if isnan(real_sigma)
