function [aligned_Data_ByStation,mean_Data] = fcn_meanStationProjection(lapData,numLaps,~)
...
index_to_match = 200; % A random point on each path to match to the first traversal
...
for i = 1:numLaps
       % data.traversal{i} = m.processGPS(data.traversal{i});% calculate, station, velocity,yaw and ENU
        ...
        data.traversal{i}.Z = data.traversal{i}.MergedGPS.zUp(inds);
        data.traversal{i}.yaw = data.traversal{i}.MergedGPS.Yaw_deg(inds);
...
for i = 1:numLaps
    ...
    all_zUp_measurements(i,:) = aligned_Data_ByStation.traversal{i}.Z'; 
    all_station_measurements(i,:) = aligned_Data_ByStation.traversal{i}.station';
    %all_yaw_measurements(i,:) = aligned_Data_ByStation.traversal{i}.yaw';
    all_yaw_measurements(i,:) = aligned_Data_ByStation.traversal{i}.yaw';  % Fixed this on 2020_05_29
    ...
    if 1==0
        ...
        hold on;
        subplot(2,1,1);
        plot(all_xEast_measurements(i,:),all_yNorth_measurements(i,:));
        subplot(2,1,2);
        plot(all_yaw_measurements(i,:));
    end
