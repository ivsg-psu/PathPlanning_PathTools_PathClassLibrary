function medianFilteredRawDataWithSigmas = fcn_medianFilterFromRawAndSigmaData(rawDataWithSigmas)
    ...
    for i_subField = 1:length(subfieldNames)
        ...
        if any(strcmp(subFieldName,fields_to_calculate_median_filters_for))
            ...
            if any(strcmp(subFieldName,fields_to_check_diff_not_zero))
                ...
                data = fcn_replaceBadIndicesWithNearestGood(data,good_indices);
                
            end
            
            % Perform median filter (Edited this section on 2019_11_27 to
            % include Sigma calculations)
            [median_cleaned_data, sigma_cleaned_data] = fcn_medianFilterData(data,2*sigmas);
            ...
            if length(sigmas)>1
                ...
                real_sigma = fcn_calcSigmaNoOutliers(median_cleaned_data);
...
function [data_median,sigma_median] = fcn_medianFilterData(data,sigmas)
