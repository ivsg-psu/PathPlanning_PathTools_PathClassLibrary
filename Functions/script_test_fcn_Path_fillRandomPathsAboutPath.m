% script_test_fcn_Path_fillRandomPathsAboutPath.m
% Tests fcn_Path_fillRandomPathsAboutPath
...
random_paths = ...
    fcn_Path_fillRandomPathsAboutPath(paths_array{1},...
    emtpy_value,... % (std_deviation),...
    emtpy_value,... % (num_paths),...
...
assert(length(random_paths)==1); %#ok<ISCL>
for ith_path = 1:length(random_paths)
    assert(isequal(size(random_paths{1}),[NreferencePoints 2]));
end
...
% Check variable sizes
NreferencePoints = length(paths_array{1}(:,1));
assert(length(random_paths)==num_paths); 
for ith_path = 1:length(random_paths)
    assert(isequal(size(random_paths{1}),[NreferencePoints 2]));
end
...
% Check variable sizes
NreferencePoints = num_points; %length(paths_array{1}(:,1));
assert(length(random_paths)==num_paths); 
for ith_path = 1:length(random_paths)
    assert(isequal(size(random_paths{1}),[NreferencePoints 2]));
end
