function cleanAndTimeAlignedData = fcn_DataClean_alignToGPSTimeAllData(cleanData)
...
%  2020_11_10 - changed name in prep for DataClean class
%
% To do items:
% (as of 2019_10_17) - clean up field checks to do this all at once (see
% fcn_DataClean_loadSigmaValuesFromRawData for an example)
...
startTimeVotesGPS(1) = fcn_DataClean_findNearestDecimatedTime(cleanData.GPS_Novatel.GPS_Time(1,1),max_decimation);
startTimeVotesGPS(2) = fcn_DataClean_findNearestDecimatedTime(cleanData.IMU_Novatel.GPS_Time(1,1),max_decimation);
startTimeVotesGPS(3) = fcn_DataClean_findNearestDecimatedTime(cleanData.GPS_Hemisphere.GPS_Time(1,1),max_decimation);
...
endTimeVotesGPS(1) = fcn_DataClean_findNearestDecimatedTime(cleanData.GPS_Novatel.GPS_Time(end,1),max_decimation);
endTimeVotesGPS(2) = fcn_DataClean_findNearestDecimatedTime(cleanData.IMU_Novatel.GPS_Time(end,1),max_decimation);
endTimeVotesGPS(3) = fcn_DataClean_findNearestDecimatedTime(cleanData.GPS_Hemisphere.GPS_Time(end,1),max_decimation);
...
startTimeVotesROS(1) = fcn_DataClean_findNearestDecimatedTime(cleanData.GPS_Novatel.ROS_Time(1,1),max_decimation);
startTimeVotesROS(2) = fcn_DataClean_findNearestDecimatedTime(cleanData.IMU_Novatel.ROS_Time(1,1),max_decimation);
startTimeVotesROS(3) = fcn_DataClean_findNearestDecimatedTime(cleanData.GPS_Hemisphere.ROS_Time(1,1),max_decimation);
cleanAndTimeAlignedData.Clocks.startTimeROS = max(startTimeVotesROS);  % Align to the GPS referenced sensor that turned on last

endTimeVotesROS(1) = fcn_DataClean_findNearestDecimatedTime(cleanData.GPS_Novatel.ROS_Time(end,1),max_decimation);
endTimeVotesROS(2) = fcn_DataClean_findNearestDecimatedTime(cleanData.IMU_Novatel.ROS_Time(end,1),max_decimation);
endTimeVotesROS(3) = fcn_DataClean_findNearestDecimatedTime(cleanData.GPS_Hemisphere.ROS_Time(end,1),max_decimation);
...
elseif (target_ROS_Npoints == (target_GPS_Npoints-2))
    ...
    cleanAndTimeAlignedData.Clocks.endTimeGPS   = cleanAndTimeAlignedData.Clocks.endTimeGPS - max_decimation;
elseif (target_ROS_Npoints == (target_GPS_Npoints-3))
    % Case 5: there are 3 more GPS points than ROS points - nudge GPS points
    % inward on both ends
    cleanAndTimeAlignedData.Clocks.startTimeGPS   = cleanAndTimeAlignedData.Clocks.startTimeGPS + max_decimation;
...
for i_data = 1:length(names)
    ...
    if isfield(d,'GPS_Time') && ~any(isnan(d.GPS_Time))
        ...
        time_to_search = fcn_DataClean_findNearestDecimatedTime(d.GPS_Time,deltaT);
        ...
        time_to_search = fcn_DataClean_findNearestDecimatedTime(d.ROS_Time,deltaT);
    ...
    if ~isfield(d,'GPS_Time') && isfield(d,'ROS_Time')
        ...
        cleanAndTimeAlignedData.(data_name).GPS_Time_Sigma =...
            fcn_DataClean_calcSigmaNoOutliers(d.ROS_Time(good_search_time_indices_intersect));
    ...
    for i_subField = 1:length(subfieldNames)
        ...
        elseif strcmp(subFieldName,'GPS_Time_Sigma')
            GPS_Time_Sigma = fcn_DataClean_calcSigmaNoOutliers(d.GPS_Time(good_search_time_indices_intersect));
        ...
        elseif strcmp(subFieldName,'ROS_Time_Sigma')
            ROS_Time_Sigma = fcn_DataClean_calcSigmaNoOutliers(d.ROS_Time(good_search_time_indices_intersect));
...
%     Vq = interp1(X,V,Xq);
%     predicted_GPS_time = fcn_DataClean_findNearestDecimatedTime(Vq,deltaT);
%
...
function goal_decimated = fcn_DataClean_findNearestDecimatedTime(goal_time,decimation)
