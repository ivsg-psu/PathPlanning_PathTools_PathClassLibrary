function fcn_plotYawFromVelocity(d,fig_number,fig_name)

x = (1:length(d.Yaw_deg_from_velocity(:,1)))';

yaw_change_due_to_navMode = d.EmptyVector;
yaw_change_due_to_navMode(d.navMode_bad_indices) = d.Yaw_deg_from_velocity(d.navMode_bad_indices);

% Calculate median filtered values for yaw
yaw_median = medfilt1(d.Yaw_deg_from_velocity,7,'truncate');

% Add bounds to the position-based yaw
highest_expected_yaw_angles_in_deg = yaw_median + 2*d.Yaw_deg_from_velocity_Sigma;
lowest_expected_yaw_angles_in_deg = yaw_median - 2*d.Yaw_deg_from_velocity_Sigma;
...
title('Plot of yaw angle from GPS velocity');
