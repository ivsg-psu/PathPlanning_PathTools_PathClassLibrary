...
% % for debugging - shows that the Novatel's velocity signal is quite good
% figure;plot(GPS_Novatel.GPS_Time-GPS_Novatel.GPS_Time(1,1),GPS_Novatel.velMagnitude);
% figure;plot(GPS_Novatel.GPS_Time-GPS_Novatel.GPS_Time(1,1),GPS_Novatel.velNorth);
% figure;plot(GPS_Novatel.GPS_Time-GPS_Novatel.GPS_Time(1,1),GPS_Novatel.velEast);
% figure;plot(GPS_Novatel.GPS_Time-GPS_Novatel.GPS_Time(1,1),GPS_Novatel.velUp);
...
GPS_Novatel.Yaw_deg_from_velocity_Sigma...
     = fcn_predictYawSigmaFromVelocity(filt_speeds,GPS_Novatel.OneSigmaPos);
...
for i_data = 1:length(sensor_names)
    ...
    if isfield(d,'ROS_Time')
        if centiSeconds ~= round(100*mean(diff(d.ROS_Time)))
            error('For sensor: %s, the centiSeconds does not match the calculated time difference in ROS_Time',sensor_name);
        end
    end
