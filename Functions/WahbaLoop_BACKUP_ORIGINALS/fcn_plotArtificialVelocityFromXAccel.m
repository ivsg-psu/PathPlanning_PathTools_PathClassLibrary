function fcn_plotArtificialVelocityFromXAccel(MergedData,cleanAndTimeAlignedData)

% Create artificial velocity
data1 = MergedData.Velocity.Velocity_Average;
time1 = cleanAndTimeAlignedData.Clocks.targetTimeVector_GPS{1};

data2 = MergedData.XAccel.XAccel_Average;
time2 = cleanAndTimeAlignedData.Clocks.targetTimeVector_GPS{1};

vel_calc = cumsum(data2)*0.01+data1(1,1);

figure(24464);
clf;
hold on;
plot(time1 - time1(1,1), data1,'k');
plot(time2 - time2(1,1), vel_calc,'r');
legend('Ave Veloicty','Calculated Velocity from X Acceleration');
...
yaw_calc2 = scale*vel_calc + a*t + b*t.^2;
...
% Finally, test the regreesion
yaw_calc3 = yaw_offset + scale*vel_calc + a*t + b*t.^2;
plot(t, yaw_calc3,'b');
legend('Median Yaw',...
    'yawPred',...
...
ones_vector = ones(length(vel_calc),1);
...
for i_start = (N_window+1):length(yaw_calc_full)
    ...
    vel_calc = yaw_calc_full(window_indices,1);  
    ...
    if 1==0
        if mod(i_start,40)==0
            local_yaw_pred = yaw_offset + scale*vel_calc + a*local_t + b*local_t_squared;
