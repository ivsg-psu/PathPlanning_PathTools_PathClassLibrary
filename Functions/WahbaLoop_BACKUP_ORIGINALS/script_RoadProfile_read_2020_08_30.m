...
addpath('..\data\');    % all the data files
addpath('..\Locate_Point_WRT_Path\');    % all the functions about Locate_Point_WRT_Path
...
flag.laps = true; % set to true if the data can be cut into laps
...
% raw data.
try
    fprintf(1,'Starting the code using variable rawData of length: %d\n', length(rawData));
catch
    if flag.DBquery == true
        ...
        % run query function 
        [result,trip_name,trip_ids] = fcn_mappingVanRawQuery(database_name,queryCondition,queryFlag);

        % pre process the data, converting data format from table to struct
        rawData = fcn_preProcessQueryResult(result);
        
    else
        
        %%define route that will be used
        trip_names = {'1.test_track','2.wahba_loop'};
        
        prompt = ['Which trip are you going to process? ' newline strjoin(trip_names,'\n') newline 'Input trip numbe[1/2/n]:'];
        ...
        elseif User_input==2
            ...
            error('Trip number error')
        end
        
        fprintf(1,'\nAre you going process this trip: %s \n',trip_name{1});
        prompt = '[y/n]';
        
        User_input = input(prompt,'s');
        
        if strcmpi(User_input,'y')
            fprintf(1,'Thanks. Let''s load it...\n');
...
% set the route name, used for laps generation
if flag.laps
    if contains(trip_name,'test track','IgnoreCase',true)
        route_name = 'test_track';
    elseif contains(trip_name,'wahba','IgnoreCase',true)
        route_name = 'wahba_loop';
    else
        error('Please set the route name, and laps perameters in fcn_defineRouteStructure()')
    end
end
