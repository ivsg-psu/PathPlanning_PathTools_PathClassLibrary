function mergedData = fcn_removeDGPSJumpsFromMergedData(mergedData,rawData)
...
pairings = fcn_findStartEndPairsWhereDGPSDrops(data_to_fit.DGPS_is_active);
...
if ~isempty(pairings)
    for i_pairing = 1:length(pairings(:,1))
        ...
        [xEast_clean,~] = fcn_medianFilterViaIncrementTemplate(xEast,xEast_increment_pred,DGPS_is_active);
        ...
        [yNorth_clean,~] = fcn_medianFilterViaIncrementTemplate(yNorth,yNorth_increment_pred,DGPS_is_active);
...
function pairings = fcn_findStartEndPairsWhereDGPSDrops(DGPS_is_active)
%%
