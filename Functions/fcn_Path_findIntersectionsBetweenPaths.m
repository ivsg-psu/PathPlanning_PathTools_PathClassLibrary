function [intersection_points,...
    ...
    varargin)
% fcn_Path_findIntersectionsBetweenPaths
% Given two paths, finds the intersection points between the traverals
...
% INPUTS:
%
%      path_1: a N x 2 or N x 3 set of coordinates
%      representing the [X Y] or [X Y Z] coordinates, in sequence, that
%      specifies the first path
%
%      path_2: a N x 2 or N x 3 set of coordinates
%      representing the [X Y] or [X Y Z] coordinates, in sequence, that
%      specifies the second path
%
%      (OPTIONAL INPUTS)
%
...
%      the M intersections between two paths. Returns an empty matrix
%      if there are no intersections
%
%      s_coordinates_in_path_1: a scalar (1x1) representing the
%      s-coordinate distance along the first path where the
%      intersection(s) take place
%
%      s_coordinates_in_path_2: a scalar (1x1) representing the
%      s-coordinate distance along the second path where the
%      intersection(s) take place
%
% DEPENDENCIES:
%
...
% EXAMPLES:
%
% See the script: script_test_fcn_Path_findIntersectionsBetweenPaths
...
if 0==flag_max_speed
    if flag_check_inputs
        ...
        fcn_DebugTools_checkInputsToFunctions(path_1, 'path2or3D');

        % Check the path_2 input
        fcn_DebugTools_checkInputsToFunctions(path_2, 'path2or3D');
...
raw_s_coordinates_in_path_1 = [];
raw_s_coordinates_in_path_2 = [];
raw_all_path_segments_1 = [];
raw_all_path_segments_2 = [];

for i_segment_path_1 = 2:length(path_1(:,1))
    sensor_vector_start = path_1(i_segment_path_1-1,:);
    sensor_vector_end   = path_1(i_segment_path_1,:);
    ...
    if ~isempty(distance) && ~all(isnan(distance))
        ...
        for ith_hit = 1:length(location(:,1))
            ...
            % Find the s-coordinate for path 1
            [~,s_coordinate_1_this_hit,first_path_point_index,second_path_point_index,percent_along_length] = ...
                fcn_Path_snapPointToPathViaVectors(point, path_1); %#ok<ASGLU>
            
            % Interpolate the s-coordinate in 1
            path_1_Station = fcn_Path_calcPathStation(path_1,-1);
            s_diff_1_this_hit = ...
                path_1_Station(i_segment_path_1) - ...
                path_1_Station(i_segment_path_1-1);
            ...
            %                 warning('Interpolated and calculated s-coordinate distances did not match. This usually indicates a poorly formed path or a situation where a path loops back onto itself.');
            %             end
            
            % Update the s-coordinates for path 1
            raw_s_coordinates_in_path_1 = [...
                raw_s_coordinates_in_path_1;
                s_coordinate_1_this_hit_interp];
            

            % Find the s-coordinate for path 2
            [~,s_coordinate_2_this_hit,first_path_point_index,second_path_point_index,percent_along_length] = ...
                fcn_Path_snapPointToPathViaVectors(point, path_2); %#ok<ASGLU>

            % Interpolate the s-coordinate in 2
            path_2_Station = fcn_Path_calcPathStation(path_2,-1);
            s_diff_2_this_hit = ...
                path_2_Station(path_segments(ith_hit)+1) - ...
                path_2_Station(path_segments(ith_hit));            
            
            % Find where the second point started
            second_point = path_2(path_segments(ith_hit),:);
            ...
            % Update the s-coordinates for path 2
            raw_s_coordinates_in_path_2 = [...
                raw_s_coordinates_in_path_2;
                s_coordinate_2_this_hit_interp];
...
if ~isempty(raw_intersection_points)
    ...
    s_coordinates_in_path_1 = cleaned_data(:,3);
    s_coordinates_in_path_2 = cleaned_data(:,4);
...
if flag_do_plots
    ...
    if ~isempty(intersection_points)
        % Plot the hit points
        plot(intersection_points(:,1),intersection_points(:,2),'ro','Markersize',20,'DisplayName','Intersections');
        ...
        for ith_hit = 1:length(intersection_points(:,1))
            text(intersection_points(ith_hit,1),intersection_points(ith_hit,2),sprintf(...
                ...
                ith_hit, ...
                s_coordinates_in_path_1(ith_hit),...
                all_path_segments_1(ith_hit),...
                s_coordinates_in_path_2(ith_hit),...
                all_path_segments_2(ith_hit)));
        end
        
        
        legend('path 1','path 2','Intersections');
