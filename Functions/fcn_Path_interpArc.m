function [pt,dudt,fofthandle] = fcn_Path_interpArc(t,px,py,varargin)
...
% theta(end+1) = theta(1); % the end point is identical with the first one, make it a closed curve,
% px = cos(theta);
% py = sin(theta);
%
...
% px = cos(theta);
% py = sin(theta);
% pz = px.*ones(length(py),1);
% % interpolate using parametric splines
% pt = interparc(100,px,py,pz,'spline');
% 
% % Plot the result
% figure(110)
% clf
% plot3(px,py,pz,'r*',pt(:,1),pt(:,2),pt(:,3),'b-o')
% axis([-1.1 1.1 -1.1 1.1])
% axis equal
% grid on
% xlabel X
...
% Revision: Change the 2D to 3D, by Liming Gao,
...
method = 'spline'; % if nargin = 3, (t,px,py)
