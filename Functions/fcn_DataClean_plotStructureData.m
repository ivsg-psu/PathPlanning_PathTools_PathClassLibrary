function fcn_DataClean_plotStructureData(rawDataWithSigmas,plottingFlags)
%fcn_DataClean_plotStructureData - this function plots the data located in
%the first argument, with plotting flags listed in the second argument.
%   
...
%   2021_01_07 
%      -- Updated the file name for DataClean class
    ...
    if strncmpi(fieldString,'All',3)
        ...
        if strncmpi(fieldString,'All_SingleSensor_',16)
            ...
            for i_plot = 1:length(sensors_that_have_this_field)
                ...
                [figNum] = fcn_DataClean_plotGenerateFigureNumber(fieldString,sensorString,structureString);
                identifierString = cat(2,sensorString,' Allsensors ',fieldString,' for ',structureString);
                fcn_DataClean_plotAllSources(rawDataWithSigmas,fields_to_use,[{sensorString}],figNum,identifierString,ylabel_string); %#ok<*NBRAK>
                fcn_DataClean_autoZoom(fieldString, plottingFlags)
            end
        elseif strncmpi(fieldString,'All_All',7)
            ...
            [figNum] = fcn_DataClean_plotGenerateFigureNumber(fieldString,sensorString,structureString);
            fcn_DataClean_plotAllSources(rawDataWithSigmas,fields_to_use,sensors_that_have_this_field,figNum,identifierString,ylabel_string);
            fcn_DataClean_autoZoom(fieldString, plottingFlags)
        else
        ...
        for i_plot = 1:length(sensors_that_have_this_field)
            ...
            if any(strcmp(sensorString,plottingFlags.SensorsToPlotIndividually))
                ...
                if ~isfield(rawDataWithSigmas,sensorString)
                    ...
                    fcn_DataClean_plotVariableWithBounds(rawDataWithSigmas.(sensorString),fields_to_use{1},figNum,identifierString, ylabel_string,identifierString);
                    fcn_DataClean_autoZoom(fieldString, plottingFlags)
                end
            end
        end
...
function fcn_DataClean_autoZoom(fieldString, plottingFlags)
